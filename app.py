...
def get_column_mapping_ui(df):
    st.subheader("Map Your Columns to Hierarchy Levels")
    columns = list(df.columns)
    default_mapping = {}
    for key in ['generation', 'transmission', 'substation', 'distribution', 'consumer']:
        # Try to auto-select a column if the keyword is in the name
        default = next((col for col in columns if key in col.lower()), None)
        default_mapping[key] = default
    mapping = {}
    for key, label in zip(['generation', 'transmission', 'substation', 'distribution', 'consumer'],
                          ['Generation', 'Transmission', 'Substation', 'Distribution', 'Consumer']):
        mapping[key] = st.selectbox(f"Select column for {label}", options=[None] + columns, index=(columns.index(default_mapping[key])+1) if default_mapping[key] else 0)
    return mapping
...
def show_hierarchy(df, mapping):
    ...
    if not all(mapping.values()):
        st.info("Cannot visualize hierarchy until all levels are mapped. Please use the dropdowns above to map your columns.")
        return
    try:
...
# --- Main App Logic ---
if data is not None:
    mapping = get_column_mapping_ui(data)
